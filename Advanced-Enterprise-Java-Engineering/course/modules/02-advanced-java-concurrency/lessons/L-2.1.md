...
- `volatile`, monitors, and safe construction are publication tools. They are not a ledger.
...
3. Does making `Payment` fields `final` remove the need to publish the `Payment` reference safely?
...
3. No. `final` fields are visible after the constructor finishes **and** the reference is published through a safe edge. A racy assignment of the reference can still leak a stale or null view.
