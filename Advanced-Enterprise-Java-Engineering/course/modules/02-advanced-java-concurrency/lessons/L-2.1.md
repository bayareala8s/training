...
Open the three pictures in [diagrams/java/l21/README.md](../../../../diagrams/java/l21/README.md). Catalog twin of picture 2 is **AEJE-D-005**.

**1. Stacks are private. The heap is shared.**

```mermaid
flowchart LR
  API[API thread stack] --> Pay[Payment on heap]
  WRK[Worker thread stack] --> Pay
```

**2. No happens-before: the worker may keep `authorized == false`.**

```mermaid
flowchart LR
  subgraph apiThread [API thread]
    A1[Write authorized = true]
    A2[Write amount = 84.00]
  end
...
**3. `volatile` write/read, monitor unlock/lock, or `Lock.unlock`/`lock` makes that dotted edge solid.** The worker then sees both fields together — not `true` paired with `0.00`.

```mermaid
flowchart LR
  W[API write] -->|volatile or unlock| HB[happens-before]
  HB -->|read or lock| R[worker sees both fields]
```

Alt text: Two BayPay threads share one Payment. Without happens-before the worker can spin on false. A volatile write or unlock/lock publishes authorized and amount. BayPay is a fictional payment platform used for instruction.
