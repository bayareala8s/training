...
**`synchronized`** provides mutual exclusion *and* visibility. Entering the block *locks* the monitor; leaving it *unlocks*. That unlock happens-before the next lock on the same monitor. Everything written before the unlock is visible to the thread that next acquires the lock. That is why a synchronized `authorize` can safely do check-then-act on an ordinary `HashMap` — not because `HashMap` is concurrent, but because only one thread is in the critical section.

**`lock()` / `unlock()`** is the same pair on `java.util.concurrent.locks.Lock`. `lock()` waits until the lock is free, then acquires. `unlock()` releases. Unlock is the happens-before; a forgotten unlock is a hang, not a compile error. Always:
