#!/usr/bin/env python3
"""Generate Stage 11 capstone diagrams D-071 and D-072."""
...
DIAGRAMS = [
    (
        ...
        "overview",
        2,
        "BayPay initial WebSphere topology",
        "flowchart LR\n  Merch[merchants] --> IHS[ihs-east]\n  IHS --> Pay[PaymentCluster]\n  IHS --> Ref[RefundCluster]\n  Pay --> DB[db-east]",
        ["merchants", "ihs-east", "PaymentCluster", "db-east"],
    ),
    (
        "AEJE-D-072",
        "capstones",
        ...
        "CAPSTONE-2",
        4,
        "Cloud-native BayPay target state",
        "flowchart LR\n  Merch[merchants TLS] --> ALB[ALB / Route]\n  ALB --> Pay[payment-service 8080]\n  Pay --> SM[secrets]\n  Pay --> DB[(teaching DB)]",
        ["TLS edge", "payment-service", "secrets", "DB"],
    ),
]
