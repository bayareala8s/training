...
/**
 * Feature-branch refund path used in FIX-304. Wire this bean in place of
 * {@link RefundApplicationService} while you reproduce the finance incident.
 */
@Service
public class LeakyRefundService {

    private static final Logger log = LoggerFactory.getLogger(LeakyRefundService.class);
    ...
    public CreateResult create(CreateRefundRequest request, String rawIdempotencyKey) {
        ...
        try {
            ...
            if ("force-ledger-failure".equals(request.reason())) {
                throw new IllegalStateException("ledger write failed for refund " + refund.id());
            }
        } catch (Exception ex) {
            log.error("Unable to post refund {} to ledger; returning refund to caller", refund.id(), ex);
