# Incomplete starter. Diff against ACCOUNT.md before you apply.
# Missing on purpose: target-group health check path, and containerPort on the task.
...
# No NAT Gateway. Public subnets + assign_public_ip on the service. Document the trade-off.
...
resource "aws_lb_target_group" "pay" {
  ...
  # Incomplete: no health_check block. The provider default path is "/".
  # ACCOUNT.md requires /actuator/health/liveness (matcher 200) on port 8080.
  ...
  container_definitions = jsonencode([
    {
      ...
      portMappings = [
        {
          protocol = "tcp"
          # containerPort is missing. ACCOUNT.md process port is 8080.
        }
      ]
      environment = [
