...
### Lab option ↔ handbook mapping

| Lab option | Handbook option | Endpoint(s) | Primary focus |
|------------|-------------------|---------------|---------------|
| **A** | Option 1 — AI Operations Assistant | `POST /capstone/incident` | Incident triage, severity, Step Functions |
| **B** | Option 2 — AI File Automation Platform | `POST /capstone/document` | Document classification and queue routing |
| **C** | Option 3 — Enterprise AI Workflow Engine | `POST /capstone/approval/request` + `/decide` | Human-in-the-loop approvals, DynamoDB state |
| **D** | Option 4 — Internal AI API Platform | `POST /capstone/agent` | Multi-tool agent router with policy + memory |

Runnable reference implementation: `labs/week08/`. Status overview: `CAPSTONE_PROJECTS.md`. Verify with `./scripts/verify-capstone.sh`.
